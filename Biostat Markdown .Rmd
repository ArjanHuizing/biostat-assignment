...
> We see that the Type 1 error is very dependent on the sample size and can vary when the sample size changes. 
...
legend("bottomright", inset=.05,
  	c("y","v"), fill=c("blue", "dark red"), horiz=FALSE)
```
...
legend("bottomright", inset=.05,
  	c("y","v", "w"), fill=c("blue", "dark red", "black"), horiz=FALSE)
```

> The power is lower for smaller n and bigger for bigger n. 
